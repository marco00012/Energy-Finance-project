...
%+sigma*quadgk(@(u)theta_x(u)*exp(-a*(T-u)),t,T);
X_Q=X;
...
% F=@(t,T1,T2) quadgk(@(u) arrayfun(@(u)Theta(t,u),u),T1,T2)+quadgk(@(t)Lambda(t),T1,T2)/(T2-T1)+X_Q*(exp(-a*T1)-exp(-a*T2))/(a*(T2-T1))+Y_Q*(exp(-b*T1)-exp(-b*T2))/(b*(T2-T1));
 F(0,T1,T2,1,1)
% F(0,T1,T2)
mean(F(0,T1,T2,1,1))
