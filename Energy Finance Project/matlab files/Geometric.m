...
gg= @(z) exp(t*(1i*z*gamma+quadgk(@(y) nu(y).*(exp(z*1i*y)-1-(z*1i*y).*(abs(y)<1)),1e-8,50)));
%-1/k*log(1-k)
path=gamrnd(shape,scale,N_sim,time_steps);
...
%gg= @(z) exp(1i*z*gamma+quadgk(@(u)arrayfun(@(u)quadgk(@(y) nu(y).*(exp(z*1i*y)-1-(z*1i*y).*(abs(y)<1)),1e-8,50),u),0,t));
